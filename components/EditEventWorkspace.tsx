...
import { Layers, Wand2, Users, ChevronRight, Home, Loader2, Save, Download } from 'lucide-react';
import Loading from './Loading';
import EventDataEditor from './EventDataEditor';
import { toPng } from 'html-to-image';

interface EditEventWorkspaceProps {
    eventId: string;
    onBackToDashboard: () => void;
}

const EditEventWorkspace: React.FC<EditEventWorkspaceProps> = ({ eventId, onBackToDashboard }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const eventDetailData = await eventRes.json();

                setUser(userData.user);

                if (eventRes.ok && eventDetailData.event) {
                    const ev = eventDetailData.event;
                    setEventData({
                        title: ev.title,
                        ...
                        tone: (ev.tone?.toLowerCase() as any) || 'casual',
                        hostName: ev.hostName
                    });
                    setSelectedTemplate(ev.templateId as TemplateId);
                    if (ev.customization) {
                        setCustomization(ev.customization);
                    }
                    setGuests(ev.guests || []);
                }
            } catch (err) {
        ...
        <div className="flex flex-col h-screen bg-if-cream">
            ...
            <header className="h-16 border-b-2 border-black bg-white flex items-center justify-between px-4 md:px-8 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <span className="font-display font-bold text-xl tracking-tight hidden md:block">Edit Project</span>
                </div>
...
export default EditEventWorkspace;
