...
const CardPreview: React.FC<CardPreviewProps> = ({ data, guestName, templateId, customization }) => {
  ...
  const renderContent = (isExport = false) => {
    // Shared classes to manage shadows and effects for export
    const shadowClass = isExport ? '' : 'shadow-lg';
    const transitionClass = isExport ? '' : 'transition-all duration-500';
    ...
    switch (templateId) {
      ...
      case TemplateId.TECH_NEON:
        return (
          <div className={`h-full w-full ${c.dark} p-8 flex flex-col justify-between ${f.body} relative overflow-hidden border-2 ${c.border} rounded-xl shadow-none`}>
            {/* Grid Background */}
            ...
            <div className="z-10 mt-2 flex justify-between items-end">
              <span className="text-[10px] text-white/50 font-mono">ID: {randomId}</span>
      ...
      case TemplateId.BOLD_BRUTAL:
        return (
          <div className={`h-full w-full ${c.bg} border-4 ${c.border} p-6 flex flex-col rounded-none relative`}>
            ...
            <div className={`flex-grow bg-white border-2 ${c.border} p-4 mb-4 ${isExport ? '' : 'shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]'}`}>
      ...
      case TemplateId.ELEGANT_SERIF:
        return (
          <div className={`h-full w-full ${c.bg} p-10 flex flex-col text-center border border-gray-200 rounded-sm`}>
      ...
      case TemplateId.RETRO_VAPOR:
        return (
          <div className={`h-full w-full bg-gradient-to-b from-indigo-200 via-purple-200 to-pink-200 p-6 flex flex-col border-4 border-white rounded-xl relative overflow-hidden`}>
            ...
            <div className="flex-grow z-10 flex flex-col justify-center items-center text-center">
              <div className={`w-24 h-24 rounded-full bg-gradient-to-tr from-yellow-300 to-pink-500 p-1 mb-4 ${isExport ? '' : 'shadow-xl animate-pulse'}`}>
            ...
            <div className={`z-10 bg-white border-2 border-black rounded-lg p-3 ${isExport ? '' : 'shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]'}`}>
      ...
      case TemplateId.CORPORATE_PRO:
        return (
          <div className={`h-full w-full bg-white border border-gray-200 p-8 flex flex-col relative rounded-lg`}>
      ...
      case TemplateId.ABSTRACT_GEO:
        return (
          <div className={`h-full w-full ${c.bg} p-8 flex flex-col relative overflow-hidden rounded-2xl`}>
            ...
            <div className={`relative z-10 bg-white/80 backdrop-blur-sm p-6 rounded-2xl border border-white h-full flex flex-col ${isExport ? '' : 'shadow-sm'}`}>
      ...
      case TemplateId.Y2K_AESTHETIC:
        return (
          <div className={`h-full w-full bg-gradient-to-br from-pink-200 to-blue-200 p-2 rounded-xl border border-white ${isExport ? '' : 'shadow-lg'}`}>
            <div className="h-full w-full bg-white/40 backdrop-blur-md rounded-lg border border-white/60 p-4 flex flex-col relative overflow-hidden">
              ...
              <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-48 border-[1px] border-white/50 rounded-full ${isExport ? '' : 'animate-[spin_10s_linear_infinite]'}`}></div>

              <div className="flex-grow flex flex-col items-center justify-center z-10 relative">
                <div className={`bg-white/80 border border-blue-300 px-4 py-1 rounded-full text-blue-500 text-xs font-bold mb-2 ${isExport ? '' : 'shadow-sm'}`}>
                  INCOMING MSG
                </div>
      ...
      case TemplateId.PAPER_CRAFT:
        return (
          <div className={`h-full w-full ${c.bg} p-6 flex flex-col shadow-inner`} style={{ backgroundImage: 'url("https://www.transparenttextures.com/patterns/cardboard-flat.png")' }}>
            <div className={`bg-white p-6 ${isExport ? '' : 'shadow-md'} transform rotate-1 flex-grow flex flex-col items-center text-center border border-gray-200`} style={{ backgroundImage: 'url("https://www.transparenttextures.com/patterns/cream-paper.png")' }}>
      ...
      default:
        return (
          <div className={`h-full w-full ${c.bg} rounded-3xl p-8 flex flex-col relative overflow-hidden ${isExport ? '' : 'shadow-2xl'}`}>
